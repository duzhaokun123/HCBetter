 ...
 * NOTE: Void != void
 *
 * @param `return` null: void
 */
fun Class<*>.findMethodsByReturnAndArgs(`return`: Class<*>?, vararg args: Class<*>): List<Method> {
    val returnName = `return`?.name ?: "void"
    val methods = declaredMethods
//    Log.d("for ${this.name} find ${args.joinToString { it.name }} -> $returnName")
    val re = mutableListOf<Method>()
    methods.forEach {
//        Log.d("has ${it.toGenericString()}")
        if ((it.returnType.name == returnName) && it.parameterTypes.contentEquals(args)) {
            re.add(it)
//            Log.d("add ${it.toGenericString()}")
        }
    }
    return re
}
